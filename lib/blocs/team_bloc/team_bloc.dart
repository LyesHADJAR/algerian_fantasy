...
class TeamBloc extends Bloc<TeamEvent, TeamState> {
  ...
  void _onLoadTeams(LoadTeams event, Emitter<TeamState> emit) {
    emit(TeamLoading());
    try {
      // In a real app, you would load teams from a repository or service
      // For now, we'll just emit the current team state
      emit(TeamUpdated(_team));
    } catch (error) {
      emit(TeamError('Failed to load teams: $error'));
    }
  }
